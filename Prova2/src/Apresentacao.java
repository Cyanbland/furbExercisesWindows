...
public class Apresentacao extends javax.swing.JFrame {
    ...
    private void jBCadastrarAlunoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBCadastrarAlunoActionPerformed	
    	String nomeAluno = jTFNomeDoAluno.getText();
    	
    	if (nomeAluno.isEmpty()) {
    		JOptionPane.showMessageDialog(null, "Insira o nome de um aluno.");
    	} 
    	else {
        	...
        	if (!nomeExiste) {
            	Aluno al = new Aluno(nomeAluno);
            	alunos.add(al);
            	JOptionPane.showMessageDialog(null, nomeAluno + " foi cadastrado com sucesso.");
        	}
        	else {
        		JOptionPane.showMessageDialog(null, "O aluno já está cadastrado.");
        	}
    	}
    ...
    private void jBIncluirAvaliacaoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBIncluirAvaliacaoActionPerformed
        Aluno al = alunos.get(alunos.size() - 1);
        
        try {
            String descricao = jTFDescricao.getText();

            
            if (jRBProva.isSelected()) {
            	float nota;
            	if (jTFNotaConceito.getText().isEmpty()) {
            		nota = 0;
            	}
            	else {
                    nota = Float.parseFloat(jTFNotaConceito.getText());
            	}
            	Prova pr = new Prova(descricao, nota);
            	al.addAvaliacao(pr);
            	JOptionPane.showMessageDialog(null, "Prova cadastrada com sucesso.");
            }
            else {
            	if (jTFNotaConceito.getText().isEmpty()) {
            		JOptionPane.showMessageDialog(null, "Insira um conceito para o trabalho.");
            	}
            	else {
                 	char conceito = jTFNotaConceito.getText().toUpperCase().charAt(0);
                	Trabalho tr = new Trabalho(descricao, conceito);
                	al.addAvaliacao(tr);
    ...
    private void jBEmitirBoletinsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBEmitirBoletinsActionPerformed
        if (alunos.size() > 0) {
            ...
            jTABoletins.setText(msg);
        }
        else {
        	JOptionPane.showMessageDialog(null, "Cadastre um aluno antes de emitir os boletins.");
        }
